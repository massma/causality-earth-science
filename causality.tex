...
\begin{figure}
  ...
  \caption{A generic graph of the earth system sequence, limited to a
    3 time sequence subset of the infinite sequence. At time steps $t$
    and $t+1$ the state space is broken up into three events: $C$,
    $E$, and $S'(t)$, which is all parts of $S$ excluding $C$ and $E$,
    which are generic processes that are subset sof the full state
    space $S$.}
  \label{fig:generic}
\end{figure}
