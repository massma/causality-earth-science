...
discovery. The scientist tests hypotheses aibout a system's nature by
...
parameters for statistical independence tests. Additionally, some
methods invoke linearity assumptions about the relationships between
processes, which is not generally true for the Earth system.
...
\begin{figure}
  ...
  \caption{A toy graph example to demonstrate basic causal theory,
    ...
    (S). In \textbf{A)} we observe all varialbes. However, in
    \textbf{B)} and \textbf{C)} we do not observe aerosol. \textbf{B)}
    is consistent with previous notation \citep{shpitser2006} for
    representing unobserved confounders between two variables (in this
    case cloud and sunlight). However for clarity we prefer the
    notation in \textbf{C)}, which explicitly names and includes the
    unobserved confounder (in this case aerosol). \textbf{C)} is
    particularly useful for earth science, where we can often name
...
specifically, the graph is consistent with the assumed Structural
...
and additionally variables not included in the causal graph
...
SCM asserts that each variable is generated from its $f$, and the
randomness is due to the exogenous variables ($U$). For example,
sources of aerosol variability not considered include anthropogenic
aerosol emission, the biosphere, fires, volcanoes, etc. For cloud,
this includes synoptic forcing, atmospheric humdity, etc. For
radiation, this includes variability of top of atmosphere radiation,
etc. For the causal graph, as drawn, we assume that all of these
sources of randomness are independent of each other. That is,
...
factors. For example, if aerosol in Figure \ref{fig:toy}(A) were instead
unobserved and exogenous, we would represent the graph instead as in
Figure \ref{fig:toy}(B), and the corresponding SCM would be:
...
\begin{figure}
  ...
  \caption{A mutilation of Figure \ref{fig:toy}(A), where we have removed
    directed causal paths from the cause (cloud) to the effect
    (surface solar radiation). There is covariability between cloud
    and surface solar radiation that is not due to the causal
    connection between cloud and surface solar radiation, but instead
    is due to aerosol's role as a common driver.}
...
independent of each other in Figure \ref{fig:mutilated-toy}, and in
Figure \ref{fig:toy}(A) all co-variability between cloud and sunlight
...
by satisfying the backdoor criterion with \textit{adjustment} on
aerosol:
