...
\begin{abstract}
  ...
  scientific regress. Unforunately, by focusing on causal discovery we
  may have missed an opportunity to apply causal fundamentals to
  communicate assumptions and organize analyses. We argue that
  encoding domain knowledge in a causal graph, rather than inferring a
  causal graph from data, is a path towards interpretable results and
  scientific progress. We apply causal fundamentals to generic graphs
  of the climate system to identify where causal inference may be most
...
can't improve our understanding with results we can't interpret, so we
need interpretable results. Interpretation requires clear
...
(using data) of causal ``links'' and directions between
variables. This is a shame, because for most application in climate
science we believe that the assumptions behind many causal discovery
algorithms are not well justified, and/or are difficult to interpret
(Section XX, also cite Friedman). In some sense, we believe that the
current use of causal inference in Earth science has us on a path
towards confusion.

The irony of this path to confusion is that the fundamentals of causal
inference are particularly well suited to clarifying assumptions and
yielding more interpretable results. The main contribution of this
paper is to demonstrate how causal graphs, a fundamental tool of
causal inference introduced in Section XX, can be used to communicate
assumptions, organize analyses, and ultimately improve scientific
understanding. Using examples, we will show how pretty much any
analysis could benefit from inclusion of a causal graph because of
their role in clarifying assumptions (Section XX). However, an added
benefit of using a causal graph is that, while they are intuitive and
easy to draw for a domain expert, they are also mathematical objects
with a corresponding rigorous theory (which we introduce in Section
XX). Using this theory, we can analyze the causal graph to determine
whether or not we can infer causal effect(s) from the data available,
potentially lending a rigorously justified ``causal'' interpretation
to our analysis. (introduce that this is done up front, to quote dave
``think a little bit before hand in your analysis'')

For many generic problems in Earth science, meeting the conditions
required for causal inference is challenging given two fundamental
issues: 1) the system evolves through time according to the underlying
dynamics, and 2) we only partially observe the state space of that
dynamical system \citep{majda-state}. So, we apply causal theory to
identify assumptions that overcome these issues in generic Earth
science systems. Specificcally, there is the potential
for applying causal inference when we can assume:
...
We hope that identifying these assumptions demonstrates how causal
inference can be applied successfuly for Earth science
understanding. However, we do not want that to detract from our other
goals of advocating for the use of causal graphs as a tool to
communicate assumptiosn and organize analysis. Causal graphs encourage
deep thought at the start of an analysis that can prevent us from
accidentally undertaking a flawed analysis. If we realize our analysis
is flawed before dissemination fo results, we wasted a bunch of time
working on the flawed analysis. But, if we do not realize until after
dissemination of results (or never), we have introduced confused into
the scientific community. We hope that this paper will increase
understanding of fundamental causal tools and shift more focus towards
using said tools. We believe wider adoption will set us on a path to
scientific clarity, rather than conusion.

(introduce guide for reviewers and authors? how causal discovery is
not justified?)
