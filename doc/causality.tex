...
demonsrate a full causal analysis starting wtih drawing graph, and
ending with the succesful calculation of the average response of
sunlight (the effect) to an intervention on cloud (the
cause). However, often we may not be able complete this last step:
estimatign the causal effects from the available data. There are
serious challenges due to unobserved confounding in generic Earth
science problems (Section \ref{sec:necess-cond-caus}). However, we
want to emphasize that, even if calcualting a causal effect is
ultimately impossible, drawing a causal graph at the beginning of an
analysis offers tremendous benfits in terms of organization and
...
matches the author's assumptions. Often from the reader's perspective,
there are more assumptions being made than we know and these
...
discussed in a precise and rigourous way.
...
use of causal graphs are research tools. Our retrospective causal
...
quantities. Keeping quantities general can help keep graphs more
intuitive and interpretable, by limiting the number of details and
nodes that one must store in their mind. However, if logic needs
clarifying, the graph can become more explit (e.g., differentiating
wind into speed, direction, and spatial distribution, both
horizontally and vertically). As graphs become more complicated, one
can leverage interactive visualization software, or use static graph
abstractions like plates \citep{bishop2006pattern}, which are
particularly well suited to reprsetning repeated structure like we
often get in spatiotemporal systems.
...
the graph. The causal graph helps interpret the field campaign's results.

However, for field campaigns like CCOPE causal graphs are particularly
useful at the planning and funding proposal stage. Such a causal graph
could be included in any field campaign proposal, improving
communication about the system and also rigorously justifying the
campaign’s observations as necessary for calculating an effect.  In
other words, the causal graph thoretically proves in some sense that
the proposed field campaign is sound. Even before the proposal, one
...
calculate our effect of interest. This analysis communicates to
proposal reviewers the campaign is rigorously optimized to minimize
cost while still tractably calculing the causal effect(s) of interest.
...
general graphs that are reprentative of many problems in Earth
...
now turn our attention to generic problems in Earth science systems,
the common challenge we may need encounter attemptign causal
inference, and how we can overcome these challenges.
...
However, still care must be taken at the human-climate interface:
human-based sources of confounding may need to be controlled for
depending on the cause of interest. For example, CO$_2$ emissiosn
...
(e.g., the COVID-19 virus is a common cause to all global human
behavior in 2020). This highlights that drawing and reasoning about
the graph is always an important first step to any analysis.
...
\item Causal graphs concisely and clearly encode assumptions about
  ...
  functioanl relationships when we either do not know the functional
...
to approximations. However, many of these developments in causal
theory are not as well established in applications. We find that the
use of causal graphs to organize and structure analyses is mature and
...
that drawing and including causal graphs in Earth science resaerch
becomes much more common, and that this manuscript provides some o
fthe the necessary foundation for readers to feel comforable using
causal graphs in their next project.
