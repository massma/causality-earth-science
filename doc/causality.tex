...
surface sunlight/solar radiation (Appendix \ref{prob-theory} describes
the notation used in this paper and a brief introduction to probability
theory for unfamiliar readers). Interpreted causally, as in
...
edge between cloud and sunlight.  We can mathematically incorporating
...
\textit{/ TODO: a messy start to compare and contrast with causal
  discovery is here; clean this up. I think pierre wanted it here.
  also ask Jakob if he wants to be a coauthor and fill this out or
  rewrite it, especially re: the motivation of causal discovery. goal
  is for readers to: 1) understand the difference between
  ``discovery'' and ``estimating effects,'' and 2) appreciate that we
  may often know the causal dependencies of physical systems, and when
  we do, ``estimating effects'' is a tractable path of analysis./}
...
\ref{fig:generic}a). For example, we generally do not observe the
...
many Earth science situations. To make the graphs more relatable, we
will occasionally refer to an example where the cause ($C(t)$) is soil
moisture and the effect ($E(t+1)$) is evapotranspiration because this
is a potentially rewarding of application for causal inference and one
that is near to our research interest. However, what we discuss is
generic to any problem in earth science where the system is assumed to
evolve according to an underlying dynamical system and associated
state.
...
another (e.g. evapotranspiration at time $t+1$), because the mapping
...
include the unknowable and observed portions of the state space. So,
...
response to global warming is relatively independent of the climate
state \citep{arto2014drivers}. We have failed to reduce green house
gas emissions even as global temperature increased. Instances of
...
\item We overcame these challenges by applying causal theory to
  generic causal graphs of the Earth system and identified the
