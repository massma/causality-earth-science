...
\begin{figure}
  ...
  \caption{A generic graph of the Earth system state sequence, limited
    ...
    (un-identifiable) without additional assumptions. At a given time
    we do not observe the entire state space (e.g., $S(t)$, $S(t-1)$),
    but we also often do not observe the Earth's state at all times:
    observations are ususally at discrete times, so at intermediate
    times (e.g. $S(t-1/2)$) we observe noneof the state space evolution.}
  \label{fig:generic}
\end{figure}
