...
in the Earth system, so have less of a need for ``causal discovery'',
but we \textit{are} often interested in inferring the functional form
of causal connections from data (i.e., a need to infer how changing X
would cause Y to change), which is not addressed with causal
discovery. In some sense, we believe that the current focus on causal
discovery in Earth science has us on a path towards confusion.
...
causally. These points require a fair bit of background and
discussion, and may have ended up a bit long winded, so here is a
table of contents so you can pick those sections that are most
relevant to you given your background knowledge and interests:
