...
introduction to causality, and is most relevant for the generic
...
estimating the causal effects from the available data, usually because there are
...
want to emphasize that, even if calculating a causal effect is
...
discussed in a precise and rigorous way.
...
graph can become more explicit (e.g., in this case differentiating
...
particularly well suited to representing repeated structure like we
...
However, for field campaigns like in Figure \ref{fig:ccope}, causal
graphs are particularly useful at the planning and proposal
stage. Such a causal graph could be included in any field campaign
proposal, improving communication about the system and also rigorously
justifying the campaign’s observations as necessary for calculating
the desired effect(s). Even before the proposal, one could start with a
causal graph, and then analyze the causal graph to determine which
observations are needed to meet the campaign’s goals. Building on this
idea, one could attach costs associated with observing each variable
in the graph, and automatically determine the set of observations that
minimizes cost while still allowing us to calculate our effect(s) of
interest. This analysis communicates to proposal reviewers the
campaign is rigorously optimized to minimize cost while still
tractably calculating the causal effect(s) of interest.
