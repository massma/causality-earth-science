...
state space? There are several classes of assumptions we can make that
may be reasonable for many generic scenarios in Earth science:

% reformulate this as table, with columns: assumption, graph,
% identification formula, commentary, test.

\begin{itemize}
\item The simplest assumptions is that \textbf{we observe all state
    variables that impact our effect(s) of interest} (Fig ??). While
  we may not observe the entire state of the system, we sometimes may
  observe the portion of the state space that affects our specific
  variable of interst. In this case, we can calculate the causal
  effect of any observation on our effect, by blocking backdoor paths
  with the observed portion of the state space. (insert
  regression/equation?).
\item While we may not observe the entire state space, \textbf{in some
    cases we may be able to reconstruct the state at any given time
    using lagged observations of the system} \citep[see Takens'
  theorem,][]{takens1981detecting}. In this case, we can examine the
  impact of changes in reconstructed state on any future variable of
  interest, or also use the reconstructed state to block backdoor
  paths and examine the effect of individual variables on future
  variables (Figure ??). A good check to determine if we were
  successful in reconstructign the state space is to test whether the
  observed variables are conditionally independent, given the
  reconstructed state variable. This approach bears similarity to the
  deconfounder approach introduced by \cite{yixin-2019}, which argues
  that causal effects can be calculated for any problem where we can
  infer a latent variable that renders the causes conditionally
  independent.
\item In some cases, we may assume that \textbf{our cause of interest
    may be independent of the systems' state evolution} (Figure
  ??). While not generally true, in some cases a variable may behave
  independent of the system's state while still causally affecting
  that state. For example, some human behavior may be approximated as
  independent fo the climate state (e.g., city planning and land use
  decisions).
\end{itemize}
