...
the system precludes many ``causal discovery'' algorithms as well (see
...
many generic scenarios in Earth science, while eliminating this
...
esimtate $\mathbb{E}(\cdot )$), and include some strategies for
testing the assumption with data.
...
Earth science. Also while the provided checks might indentify when
assumptions break down, there is no general way to ``validate''
assumptions using data. Phsyical and science-based justification and
reasoning are ultimately always required.

\section{Conclusions}

In summary, causal graphs and causal inference are powerful tools to
reason about problems in the Earth system. Specifically, this review
...
\item Because the Earth system and its constituents are dynamical
  ...
  which would not be possible with associations/correlation/naive
  regression. However, causal inference in Earth science also presents
  challenges: we only partially observe the state space of the system.
