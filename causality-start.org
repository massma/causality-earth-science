...
** Applications in earth science: pitfalls and justified approaches for generic scenarios
    # reorganize as: what does it take to block backdoor paths. okay,
    # what about the fact that we partially observe state space?

    # also, start with discussing internal to the system, then bring
    # up how we might be able to get away with assuming soem pieces
...
*** Tractable approach: causes are independent if the state space
    ...
    present and near future /discuss data problem - we know don't
...
*** Tractable approach: thoughtfull estimation of the state space with Takens' theorem
    ...
    (from $t=0$ /introduce "lightcone" idea from shalizi?/ hr to $t=1$
    ...
    think about how to handle SM with large "memory"/. /what about
